...
namespace Starwars
{
    class Program
    {
        static void Main(string[] args) 
        {
            ...
            // Opgave 1
            //var result = from planet in planets where planet.Name[0].Equals('M') select planet;

            // Opgave 2 (I used the equal) 
            //var result = from planet in planets where !planet.Name.IndexOfAny("yY".ToCharArray()).Equals(-1) select planet;

            // Opgave 3
            //var result = from planet in planets where planet.Name.Length > 9 && planet.Name.Length < 15 select planet;

            // Opgave 4
            //var result = from planet in planets where planet.Name[1].Equals('a') && planet.Name.Last().Equals('e') select planet;

            // Opgave 5
            //var result = from planet in planets where planet.RotationPeriod > 40 orderby planet.RotationPeriod select planet;

            // Opgave 6
            //var result = from planet in planets where planet.RotationPeriod > 10  && planet.RotationPeriod < 20 orderby planet.Name select planet;

            // Opgave 7
            //var result = from planet in planets where planet.RotationPeriod > 30  orderby planet.Name, planet.RotationPeriod select planet;

            // Opgave 8
            //var result = from planet in planets where (planet.RotationPeriod < 30 || planet.SurfaceWater > 50) && planet.Name.Contains("ba")  
            //             orderby planet.Name, planet.SurfaceWater, planet.RotationPeriod select planet;

            // Opgave 9
            //var result = from planet in planets where planet.SurfaceWater > 0  orderby planet.SurfaceWater descending select planet;

            // Opgave 10
            //var result = from planet in planets where planet.Diameter > 0 && planet.Population > 0 
            //             orderby 4*Math.PI * Math.Pow(planet.Diameter/2, 2) / planet.Population ascending select planet;

            // Opgave 11
            //var result = from planet in planets.Except(from planet in new List<Planet>(planets) where planet.RotationPeriod > 0 select planet) select planet;

            // Opgave 12

            //var listA = from planet in planets where planet.Name[0].Equals('A') || planet.Name.Last().Equals('s') select planet;
            //var listB = from planet in planets where planet.Terrain != null && planet.Terrain.Contains("rainforests") select planet;
            //var result = new List<Planet>(listA.Union(listB));

            // Opgave 13 - Find alle elementer, hvor klimaet består af ørken
            //var result = from planet in planets where planet.Terrain != null && (planet.Terrain.Contains("desert") || planet.Terrain.Contains("deserts")) select planet;

            // Opgave 14 - Find alle elementer, hvor klimaet består af sump og sorter listen efter rotationsperiode og navn
            //var result = from planet in planets where planet.Terrain != null && (planet.Terrain.Contains("swamp") || planet.Terrain.Contains("swamps")) orderby planet.RotationPeriod, planet.Name select planet;

            // Opgave 15 - Find alle elementer, hvor der indgår en dobbelt vokal i navnet på planeten
            //var result = from planet in planets where Regex.IsMatch(planet.Name, "(aa|ee|ii|oo|uu|yy)") select planet;

            // Opgave 16 - Find alle elementer, hvor der indgår ”kk”, ”ll”, ”rr” eller ”nn”,i navnet på planeten og sorter listen efter navnet på planeten i faldende orden.
            var result = from planet in planets where Regex.IsMatch(planet.Name, "(kk|ll|rr|nn)") orderby planet.Name descending select planet;
